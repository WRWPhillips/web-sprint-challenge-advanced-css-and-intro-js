...
function get20s(array) {
  ...
  for(let i = 0; i < array.length; i++) {
    let transitoryArray = array[i].years.split(" - ");
    if(transitoryArray[0] >= 1900 && transitoryArray[1] >= 1900) {
...
function addArtist(array, entry) {
  array.push(entry);
  return array;
}
const imAnArtist = {   
...
bio: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam commodo pulvinar nibh sit amet pharetra."
};
...
function lotsOfArt(array) {
  let plentifulArtists = [];
  for(let i = 0; i < array.length; i++) {
    if(array[i].paintings > 100) {
      plentifulArtists.push(array[i].name);
    }
  }
  return plentifulArtists;
}
